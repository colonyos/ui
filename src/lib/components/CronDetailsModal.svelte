		...
		<div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-hidden">
			...
			<div class="px-6 py-4 border-b border-gray-200">
				<div class="flex justify-between items-start">
					<div>
						<h3 class="text-lg font-semibold text-gray-900">Cron Job Details</h3>
						{#if cron}
							<p class="text-sm text-gray-600 mt-1">{cron.name}</p>
							<p class="text-xs text-gray-400 font-mono">{cron.cronid}</p>
						{/if}
					</div>
					<button
						onclick={onClose}
			...
			<div class="px-6 py-4 overflow-y-auto max-h-[60vh]">
				{#if loadingStatus === 'loading'}
					<div class="flex items-center justify-center py-8">
						...
						<span class="text-gray-600">Loading cron details...</span>
					</div>
					...
					<div class="space-y-6">
							...
							<div class="bg-gray-50 rounded-lg p-4 space-y-2">
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
									<div>
										...
										<span class="text-gray-900 ml-2">{cronDetails.name}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Colony:</span>
										<span class="text-gray-900 ml-2">{cronDetails.colonyname}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Schedule:</span>
										<span class="text-gray-900 ml-2 font-mono">{cronDetails.cronexpression}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Initiator:</span>
										<span class="text-gray-900 ml-2">{cronDetails.initiatorname}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Next Run:</span>
										<span class="text-gray-900 ml-2">{cronDetails.nextrun ? new Date(cronDetails.nextrun).toLocaleString() : 'Not scheduled'}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Last Run:</span>
										<span class="text-gray-900 ml-2">{cronDetails.lastrun ? new Date(cronDetails.lastrun).toLocaleString() : 'Never'}</span>
									</div>
								</div>
							</div>
						</div>
						...
						<div>
							...
							<div class="bg-blue-50 rounded-lg p-4 space-y-3">
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
									<div>
										<span class="font-medium text-blue-700">Random:</span>
										<span class="text-blue-900 ml-2">{cronDetails.random ? 'Yes' : 'No'}</span>
									</div>
									<div>
										<span class="font-medium text-blue-700">Checker Period:</span>
										<span class="text-blue-900 ml-2">{cronDetails.checkerperiod ? formatDuration(cronDetails.checkerperiod / 1000) : 'Not set'}</span>
									</div>
									<div>
										<span class="font-medium text-blue-700">Wait for Previous:</span>
										<span class="text-blue-900 ml-2">{cronDetails.waitforprevprocessgraph ? 'Yes' : 'No'}</span>
									</div>
									<div>
										<span class="font-medium text-blue-700">Interval:</span>
										<span class="text-blue-900 ml-2">{cronDetails.interval && cronDetails.interval > 0 ? formatDuration(cronDetails.interval) : 'Not set'}</span>
									</div>
								</div>
							</div>
						</div>
