<script lang="ts">
	...
	let functionsLoadingStatus: 'idle' | 'loading' | 'success' | 'error' = $state('idle');
	...
	async function loadExecutorDetails() {
		...
		try {
			...
			const colonyName = (executor as any).colonyname || executor.colonyid || 'default';

			const result = await client.getExecutor(colonyName, executor.executorname);
	...
	async function loadExecutorFunctions() {
		...
		try {
			...
			const result = await client.getFunctionsForExecutor(colonyName, executor.executorname);
	...
	function formatMemory(mem: string | undefined): string {
		if (!mem) return 'Unknown';
		// If it's already formatted (e.g., "16GB"), return as is
		if (mem.includes('GB') || mem.includes('MB')) return mem;
		// Otherwise, assume it's in bytes and format it
		const bytes = parseInt(mem);
		if (isNaN(bytes)) return mem;
