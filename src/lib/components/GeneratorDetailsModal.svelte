<script lang="ts">
	...
	import { getGeneratorTriggerLabel, getGeneratorStatus } from '$lib/types/generator';
	...
	let { show, generator, client, onClose }: Props = $props();
	...
	let generatorDetails: GeneratorDetails | null = $state(null);
	let workflowSpec: WorkflowSpec | null = $state(null);

	$effect(() => {
		if (show && generator && client) {
			loadGeneratorDetails();
		}
	});

	async function loadGeneratorDetails() {
		if (!generator || !client) return;
		...
		try {
			const result = await client.getGenerator(generator.generatorid);
			generatorDetails = result;
	...
	function formatDuration(seconds: number): string {
		if (seconds === 0 || seconds === -1) return 'No limit';
	...
	function formatDate(dateString: string): string {
		if (!dateString || dateString.startsWith('0001-01-01T00:')) {
			return 'Never';
		}
		try {
