		...
		try {
			...
			if (colonies.length > 0) {
				try {
					...
					if (colonyName) {
						// Fetch 100 executors for accurate statistics
						executors = await colonyClient.getExecutors(colonyName, 100);

						// Get processes with different states (100 per state for accurate statistics)
						const runningProcesses = await colonyClient.getProcesses(colonyName, 100, 1); // Running
						const queuedProcesses = await colonyClient.getProcesses(colonyName, 100, 0); // Waiting
						const successfulProcesses = await colonyClient.getProcesses(colonyName, 100, 2); // Successful
						const failedProcesses = await colonyClient.getProcesses(colonyName, 100, 3); // Failed
