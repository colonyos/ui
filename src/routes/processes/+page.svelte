<script lang="ts">
	...
	import { sampleProcesses } from '$lib/data/sampleProcesses';
	...
	let selectedWorkflow = $state<string>(initialState.selectedWorkflow);
	...
	let removingStatus = $state<'idle' | 'removing' | 'success' | 'error'>('idle');
	...
	onMount(async () => {
		...
		if (host && port) {
			...
			const generalPrivateKey = $appState.prvKey || envConfig.prvKey || colonyPrivateKey;
			...
			if (Array.isArray(coloniesResult)) {
				...
				const processPromises = colonies.flatMap(colony => 
					stateOptions.map(async (stateOption) => {
						try {
							const processes = await colonyClient!.getProcesses(colony.name, 100, stateOption.value);
							return Array.isArray(processes) ? processes : [];
						} catch (error) {
							console.warn(`Failed to get processes for ${colony.name} state ${stateOption.label}:`, error);
							return [];
						}
					})
				);
	...
	// Get unique workflows using $derived
	let workflows = $derived(
		[...new Set(allProcesses.map(p => p.processgraphid).filter(id => id && id !== ''))]
	);

	let filteredProcesses = $derived.by(() => {
		return allProcesses.filter(p => {
			const stateMatch = selectedState !== '' ? p.state === selectedState : true;
			const workflowMatch = selectedWorkflow !== '' ? p.processgraphid === selectedWorkflow : true;
			return stateMatch && workflowMatch;
		});
	});
	...
	let groupedProcesses = $derived.by(() => {
		if (groupByWorkflow) {
			return filteredProcesses.reduce((groups, process) => {
	...
	async function confirmRemoveAllProcesses() {
		...
		try {
			const targetState = removeState !== '' ? Number(removeState) : PROCESS_STATE_NOTSET;
	...
	function getRemovalDescription(): string {
		...
		const state = removeState !== '' ? stateOptions.find(s => s.value === Number(removeState))?.label || 'unknown state' : 'all states';
...
<div class="space-y-6">
	<div>
		...
		<p class="mt-2 text-gray-600">
			Manage and monitor your process executions. View status, resource usage, and dependency
			relationships.
		</p>
	</div>
	...
	<div class="bg-white rounded-lg border border-gray-200 p-4">
		<div class="flex justify-between items-center mb-4">
			<h2 class="text-lg font-semibold text-gray-900">Process Overview</h2>
			
			<div class="flex items-center gap-4">
				<!-- State Filter -->
				<select bind:value={selectedState} class="text-sm border border-gray-300 rounded px-3 py-1">
				...
				{#if workflows.length > 0}
					<select bind:value={selectedWorkflow} class="text-sm border border-gray-300 rounded px-3 py-1">
						<option value="">All Workflows</option>
						{#each workflows as workflowId}
							<option value={workflowId}>{workflowId.substring(0, 12)}...</option>
						{/each}
					</select>
				{/if}
		...
		{:else}
			...
			<button
				...
				class:bg-blue-100={selectedState === ProcessState.RUNNING}
				class:ring-2={selectedState === ProcessState.RUNNING}
				class:ring-blue-300={selectedState === ProcessState.RUNNING}
				class="flex items-center hover:bg-blue-50 px-2 py-1 rounded transition-colors cursor-pointer"
			>
				<div class="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
				<span>Running: {allProcesses.filter((p) => p.state === ProcessState.RUNNING).length}</span>
			</button>
			<button
				onclick={() => filterByState(ProcessState.WAITING)}
				class:bg-yellow-100={selectedState === ProcessState.WAITING}
				class:ring-2={selectedState === ProcessState.WAITING}
				class:ring-yellow-300={selectedState === ProcessState.WAITING}
				class="flex items-center hover:bg-yellow-50 px-2 py-1 rounded transition-colors cursor-pointer"
			>
				<div class="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
				<span>Waiting: {allProcesses.filter((p) => p.state === ProcessState.WAITING).length}</span>
			</button>
			<button
				onclick={() => filterByState(ProcessState.SUCCESS)}
				class:bg-green-100={selectedState === ProcessState.SUCCESS}
				class:ring-2={selectedState === ProcessState.SUCCESS}
				class:ring-green-300={selectedState === ProcessState.SUCCESS}
				class="flex items-center hover:bg-green-50 px-2 py-1 rounded transition-colors cursor-pointer"
			>
				<div class="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
				<span>Success: {allProcesses.filter((p) => p.state === ProcessState.SUCCESS).length}</span>
			</button>
			<button
				onclick={() => filterByState(ProcessState.FAILED)}
				class:bg-red-100={selectedState === ProcessState.FAILED}
				class:ring-2={selectedState === ProcessState.FAILED}
				class:ring-red-300={selectedState === ProcessState.FAILED}
				class="flex items-center hover:bg-red-50 px-2 py-1 rounded transition-colors cursor-pointer"
			>
				<div class="w-3 h-3 bg-red-500 rounded-full mr-2"></div>
				<span>Failed: {allProcesses.filter((p) => p.state === ProcessState.FAILED).length}</span>
			</button>
			<button
				onclick={() => filterByState('')}
				class:bg-purple-100={selectedState === ''}
				class:ring-2={selectedState === ''}
				class:ring-purple-300={selectedState === ''}
					...
					<div class="flex justify-between items-center">
						<div>
							<h3 class="text-lg font-semibold text-gray-900">
								...
								{:else}
									Workflow: {workflowId.substring(0, 16)}...
								{/if}
							</h3>
						...
						{#if workflowId !== 'no-workflow'}
							<div class="text-xs text-gray-500">
								<div class="flex items-center gap-2">
									<span class="inline-flex px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">
										Workflow
									</span>
									<span>Dependencies: {processes.filter(p => p.parents.length > 0).length} processes</span>
								</div>
							</div>
						{/if}
					</div>
		...
		{/each}
		{:else}
			<ProcessTable processes={filteredProcesses} onProcessClick={handleProcessClick} />
		{/if}
	{/if}
	</div>
</div>
...
{#if showRemoveDialog}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
		<div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
			<div class="mb-4">
				<h3 class="text-lg font-semibold text-gray-900 mb-2">Confirm Process Removal</h3>
				<p class="text-gray-600 mb-4">
					This action will permanently remove processes and cannot be undone.
				</p>
				<div class="bg-yellow-50 border border-yellow-200 rounded p-3 mb-4">
					<p class="text-sm text-yellow-800">
						<strong>Action:</strong> {getRemovalDescription()}
					</p>
				</div>
			</div>
			...
			<!-- State Selection -->
			<div class="mb-6">
				<label class="block text-sm font-medium text-gray-700 mb-2">Process State</label>
				<select bind:value={removeState} class="w-full border border-gray-300 rounded px-3 py-2">
					<option value="">All States</option>
					{#each stateOptions as state}
						<option value={state.value}>{state.label}</option>
					{/each}
			...
			{#if removingStatus === 'error'}
				<div class="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded mb-4">
					<strong>Error:</strong> {removeError}
				</div>
			{/if}

			<!-- Success Display -->
			{#if removingStatus === 'success'}
				<div class="bg-green-50 border border-green-200 text-green-700 px-3 py-2 rounded mb-4">
			...
			<div class="flex justify-end space-x-3">
				<button
					...
					class="px-4 py-2 border border-gray-300 rounded text-gray-700 hover:bg-gray-50 disabled:opacity-50"
				>
					Cancel
				</button>
				<button
					onclick={confirmRemoveAllProcesses}
					disabled={removingStatus === 'removing' || removingStatus === 'success'}
